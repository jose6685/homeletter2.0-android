@echo off
setlocal
copy /Y ".\frontend\config.safe.js" ".\frontend\config.js" >nul 2>&1
if %errorlevel% equ 0 (
  echo 已切換為「安全模式」：暫停廣告與付費 API。請重新整理瀏覽器。
  ...
  if exist ".git" (
    ...
    if %errorlevel% equ 0 (
      ...
      if %errorlevel% equ 0 (
        git commit -m "chore(toggle): pause ads (SAFE_MODE=true)" || echo 無需提交或提交失敗
      ) else (
        echo 無變更需要提交。
      )
